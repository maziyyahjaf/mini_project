...
public class EmotionInsightsQueries {
    ...
    public static final String TIME_OF_DAY_ANALYSIS = 
            """
                SELECT CASE
                        WHEN HOUR(CONVERT_TZ(e.timestamp, @@session.time_zone, ?)) BETWEEN 0 AND 4 THEN 'late night'
                        WHEN HOUR(CONVERT_TZ(e.timestamp, @@session.time_zone, ?)) BETWEEN 5 AND 7 THEN 'early morning'
                        WHEN HOUR(CONVERT_TZ(e.timestamp, @@session.time_zone, ?)) BETWEEN 8 AND 11 THEN 'morning'
                        WHEN HOUR(CONVERT_TZ(e.timestamp, @@session.time_zone, ?)) BETWEEN 12 AND 16 THEN 'afternoon'
                        WHEN HOUR(CONVERT_TZ(e.timestamp, @@session.time_zone, ?)) BETWEEN 17 AND 20 THEN 'evening'
                        ELSE 'night'
                    END AS time_of_day,
                    el.emotion,
                    COUNT(el.log_id) AS frequency
    ...
    public static final String GET_PAST_LOGS = 
        """
            ...
            FROM emotion_logs el 
            WHERE el.firebase_user_id = ?
		    AND CONVERT_TZ(el.timestamp, @@session.time_zone, '+08:00') >= CURDATE() - INTERVAL ? DAY
            GROUP BY log_date, el.emotion
            ORDER BY log_date ASC, frequency DESC
